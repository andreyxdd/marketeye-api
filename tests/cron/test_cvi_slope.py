...
async def test_cvi_slope_handles_adv_only_day(mongo_client):
    # Calendar day prior to FIXTURE_DATE is often a weekend (not in fixtures).
    # Must restore so session-scoped mongo does not leak an extra get_dates day.
    skew_date = get_past_date(1, FIXTURE_DATE)
    epoch = get_epoch(skew_date)
    collection = mongo_client[MONGO_DB_NAME]["analytics"]
    prior = await collection.find({"date": epoch, "market": "US"}).to_list(length=10000)
    ...
    try:
        ...
        await collection.delete_many({"date": epoch, "market": "US"})
        if prior:
            await collection.insert_many(prior)
