...
from core.data import ANALYTICS_CRITERIA, BOUNCE_CRITERIA, N_BOUNCE_PERIODS
...
def get_money_flow_ratio(typical_prices, volumes, n_days) -> float:
    ...
    Args:
        ...
        n_days (int): the period, over which to compute the MFR
...
def compute_single_ticker_analytics(
    df, n_trading_days: Optional[int] = 15
):  # pylint: disable=R0914:
    """
    Function to assemble a json with a single ticker analytics
    ...
    acumulative_open_close_pct_change_for_last_N_periods = one_day_open_close_change.loc[
        ::-1, "one_day_open_close_change"
    ].cumsum().to_list()[1:N_BOUNCE_PERIODS+1]

    acumulative_open_close_pct_change_for_last_N_periods = [
        value * 100
        for value in acumulative_open_close_pct_change_for_last_N_periods
    ]
    ...
    return {
        ...
        **loads(concat(frames, join="inner", axis=1).fillna(0).iloc[-1].to_json()),
        ...
        "bounce": acumulative_open_close_pct_change_for_last_N_periods,
        ...
        "ema50": ema50[-1],
        "frequencies": {
            "analytics": dict(((criterion, "") for criterion in ANALYTICS_CRITERIA)),
            "bounce": dict(((str(criterion), list(str("") * N_BOUNCE_PERIODS)) for criterion in BOUNCE_CRITERIA)),
        },
    }
